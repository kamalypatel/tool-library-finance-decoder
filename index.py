...
k = 2
while (i <= 5773):
    if sheetread["H"+str(i)].value == "Credit/Debit (Stripe)" or sheetread["H"+str(i)].value == "Cash":
        ...
        value = sheetread["H" + str(i)].value
        if value == "Credit/Debit (Stripe)":
            writer = "Stripe"
            writertwo = "Undeposited funds"
        else:
            writer = "Cash"
        ...
        j = i-1
        check = sheetread["Q" + str(j)].value
        if check == "Membership Fee":
            checktwo = sheetread["X" + str(j)].value
        ...
        if check == "NYS & Erie County Sales Tax":
            checkagain = sheetread["Q" + str(j-1)].value
        ...
        checkagain = sheetread["Q" + str(j)].value
        if checkagain == "Cart Item Purchase":
            checktwo = "Cart Item Purchase"
            sheetwrite["J" + str(k)] = checktwo
            checkthree = "Merchandise:Cart Items for sale"
